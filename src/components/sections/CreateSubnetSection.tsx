  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* STEP 1 */}
        <div className="rounded-xl bg-white border border-slate-200 p-6 card-shadow flex flex-col justify-between hover:border-indigo-300 transition-all">
          <div>
            <div className="flex items-center justify-between mb-4">
              <span className="w-8 h-8 rounded-lg bg-indigo-100 text-indigo-600 font-mono font-bold text-sm flex items-center justify-center border border-indigo-200">
                01
              </span>
            ...
            <p className="text-sm text-slate-500 leading-relaxed mb-4">
              Select a private RFC 1918 base network (e.g. <code className="text-indigo-600 bg-slate-50 px-1 py-0.5 rounded font-mono text-xs">192.168.0.0/16</code>) and divide it into subnets based on required host capacity and isolation goals.
            </p>

            <div className="space-y-2 text-xs font-mono bg-slate-50 p-3 rounded-lg border border-slate-200 text-slate-900">
              <div className="flex justify-between border-b border-slate-200 pb-1.5">
                <span className="text-emerald-600">VLAN 10 (Staff):</span>
                <span>192.168.10.0/24 (254 hosts)</span>
              </div>
              <div className="flex justify-between border-b border-slate-200 pb-1.5 pt-0.5">
                <span className="text-amber-600">VLAN 20 (Guest):</span>
          ...
          <div className="mt-4 pt-3 border-t border-slate-200 text-xs text-slate-500">
            💡 Always reserve <code className="text-slate-900">.0</code> (Network), <code className="text-slate-900">.1</code> (Default Gateway), and <code className="text-slate-900">.255</code> (Broadcast).
          </div>
        </div>
        ...
        <div className="rounded-xl bg-white border border-slate-200 p-6 card-shadow flex flex-col justify-between hover:border-indigo-300 transition-all">
          <div>
            <div className="flex items-center justify-between mb-4">
              <span className="w-8 h-8 rounded-lg bg-emerald-500/20 text-emerald-600 font-mono font-bold text-sm flex items-center justify-center border border-emerald-200">
                02
              </span>
              <span className="text-xs font-mono text-slate-500 bg-slate-50 px-2.5 py-1 rounded border border-slate-200">
            ...
            <div className="relative">
              <pre className="bg-slate-50 border border-slate-200 rounded-lg p-3 font-mono text-xs text-slate-900 overflow-x-auto leading-relaxed">
                {ciscoCode}
              </pre>
              <button
                onClick={() => handleCopy(ciscoCode, 2)}
                className="absolute top-2 right-2 px-2 py-1 bg-[#21262d] hover:bg-[#30363d] text-slate-500 hover:text-slate-900 rounded text-xs transition-colors border border-slate-200"
              >
        ...
        <div className="rounded-xl bg-white border border-slate-200 p-6 card-shadow flex flex-col justify-between hover:border-indigo-300 transition-all">
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="w-8 h-8 rounded-lg bg-[#ffa657]/20 text-amber-600 font-mono font-bold text-sm flex items-center justify-center border border-amber-200">
                03
              </span>
