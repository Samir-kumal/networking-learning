...
const DECISION_STYLES: Record<ZeroTrustEvaluation["decision"], string> = {
  ...
  STEP_UP: "border-amber-200 bg-amber-50 text-amber-700",
  ...
  return (
    <section id="sec-zero-trust" className="scroll-mt-20 space-y-6">
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <h3 className="mt-2 text-lg font-bold text-slate-900">Zero Trust policy path</h3>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <form onSubmit={handleEvaluate} className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label htmlFor="sec-zero-trust-source" className="mb-1 block text-xs font-semibold text-slate-700">Source zone</label>
              <select
              ...
              <p className="mt-1 text-[11px] text-slate-500">{SOURCE_OPTIONS.find((option) => option.value === sourceZone)?.description}</p>
            </div>
            <div>
              <label htmlFor="sec-zero-trust-destination" className="mb-1 block text-xs font-semibold text-slate-700">Destination zone</label>
              <select
              ...
              <p className="mt-1 text-[11px] text-slate-500">{DESTINATION_OPTIONS.find((option) => option.value === destination)?.description}</p>
            </div>
            <div className="sm:col-span-2">
              <label htmlFor="sec-zero-trust-action" className="mb-1 block text-xs font-semibold text-slate-700">Requested action</label>
              <select
              ...
              <p className="mt-1 text-[11px] text-slate-500">{ACTION_OPTIONS.find((option) => option.value === action)?.description}</p>
            </div>
          </div>

          <fieldset className="space-y-2">
            <legend className="mb-2 text-xs font-semibold text-slate-700">Trust signals</legend>
            <label htmlFor="sec-zero-trust-identity" className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
              <input
              ...
              />
              <span>
                <span className="block font-semibold text-slate-800">Identity verified</span>
                <span className="mt-0.5 block text-[11px] text-slate-500">The principal is authenticated and mapped to a known identity.</span>
              </span>
            </label>
            <label htmlFor="sec-zero-trust-device" className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
              <input
              ...
              />
              <span>
                <span className="block font-semibold text-slate-800">Device is compliant</span>
                <span className="mt-0.5 block text-[11px] text-slate-500">Posture, patch, and endpoint protection checks are current.</span>
              </span>
            </label>
            <label htmlFor="sec-zero-trust-mfa" className="flex cursor-pointer items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-700">
              <input
              ...
              />
              <span>
                <span className="block font-semibold text-slate-800">MFA completed</span>
                <span className="mt-0.5 block text-[11px] text-slate-500">A recent step-up challenge is available for sensitive destinations.</span>
              </span>
            </label>
          </fieldset>

          <button type="submit" className="w-full rounded-lg bg-violet-700 px-4 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-violet-800 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-2">
          ...
          {evaluation ? (
            <div role="status" aria-live="polite" className={`rounded-lg border p-4 ${DECISION_STYLES[evaluation.decision]}`}>
              <div className="flex flex-wrap items-center justify-between gap-2">
                <span className="text-xs font-semibold uppercase tracking-wide text-slate-600">Policy decision</span>
                <span className="rounded-full bg-white/80 px-3 py-1 text-sm font-bold">{decisionLabel(evaluation.decision)}</span>
              </div>
              <dl className="mt-3 space-y-2 text-xs">
                ...
                <div>
                  <dt className="font-semibold text-slate-700">Requested action</dt>
                  <dd className="mt-0.5 text-slate-600">{actionLabel}</dd>
                </div>
              </dl>
            </div>
          ) : (
        ...
        <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div>
            <h4 className="text-sm font-bold text-slate-900">Request path visualization</h4>
            <p className="mt-1 text-xs leading-relaxed text-slate-500">Static policy stages make the trust boundary explicit before any decision is applied.</p>
          </div>
            ...
            {PATH_CARDS.map((card, index) => (
              <div key={card.key} className="contents">
                <article className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <span className="block text-[10px] font-semibold uppercase tracking-wide text-slate-500">{card.title}</span>
                  ...
                  <span className="mt-1 block text-[11px] leading-relaxed text-slate-500">{card.detail}</span>
                </article>
                {index < PATH_CARDS.length - 1 ? <span aria-hidden="true" className="hidden items-center justify-center text-lg font-semibold text-slate-300 md:flex">→</span> : null}
              </div>
            ))}
          </div>
          <div className="rounded-lg border border-violet-100 bg-violet-50/70 p-4 text-xs text-violet-900">
            <div className="flex flex-wrap items-center justify-between gap-2">
