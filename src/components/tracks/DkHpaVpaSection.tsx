  ...
  return (
    <label className="block">
      <span className="block text-xs font-mono text-slate-500 uppercase mb-2">
        {label} <span className="text-slate-400 normal-case">({unit})</span>
      </span>
      <input
        type="number"
        min={min}
        ...
        className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm font-mono text-slate-900 focus:outline-none focus:border-sky-400 focus:ring-1 focus:ring-sky-400"
      />
    </label>
  );
}
...
export default function DkHpaVpaSection() {
  ...
  return (
    <section id="hpa-vpa" className="scroll-mt-24 space-y-8 bg-white border border-sky-200 rounded-2xl p-6 sm:p-8 card-shadow">
      {/* ======================= HEADER ======================= */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-sky-100 pb-5">
        <div>
          <div className="text-xs font-mono text-sky-600 uppercase tracking-wider mb-1 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-sky-500 inline-block"></span> Module 5 • Kubernetes Workload Autoscaling
          </div>
          <h2 className="text-2xl font-extrabold text-slate-900">HPA & VPA Autoscaling Control Room</h2>
          <p className="text-xs text-slate-500 mt-1">Configure metrics, tune scale behaviors, then watch replica decisions play out live</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono px-2.5 py-1 rounded-lg bg-sky-50 border border-sky-200 text-sky-700">autoscaling/v2</span>
          <span className="text-xs font-mono px-2.5 py-1 rounded-lg bg-blue-50 border border-blue-200 text-blue-700">autoscaling.k8s.io/v1</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Controls */}
        <div className="rounded-xl bg-white border border-sky-200 card-shadow overflow-hidden">
          <div className="px-5 py-3 border-b border-sky-100 bg-gradient-to-r from-sky-50 to-blue-50/40 flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-base">📈</span>
              <div>
                <h3 className="text-sm font-bold text-slate-900">HPA Metric Configurator</h3>
                <p className="text-[11px] text-slate-500">Pick the signal the controller watches</p>
              </div>
            </div>
            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-sky-100 text-sky-700">HorizontalPodAutoscaler</span>
          </div>

          <div className="p-5 space-y-5">
            {/* Metric source */}
            <div>
              <span className="block text-xs font-mono text-slate-500 uppercase mb-2">Metric Source</span>
                ...
                {(
                  ...
                  <button
                    ...
                    className={`px-2 py-2 rounded-lg border text-left transition-all ${
                      metricKind === m.key ? "bg-sky-50 border-sky-300 ring-1 ring-sky-500" : "bg-white border-slate-200 hover:border-sky-300"
                    }`}
                  >
                    <div className={`text-[11px] font-bold ${metricKind === m.key ? "text-sky-700" : "text-slate-900"}`}>{m.label}</div>
            ...
            {metricKind === "resource" && (
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="block text-xs font-mono text-slate-500 uppercase mb-2">Resource</span>
                    ...
                    {(["cpu", "memory"] as const).map((r) => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-lg border text-xs font-mono transition-all ${
                          resourceName === r ? "bg-sky-500 text-white border-sky-500" : "bg-white border-slate-200 text-slate-700 hover:border-sky-300"
                        }`}
                      >
                        {r}
                      </button>
                ...
                <div>
                  <span className="block text-xs font-mono text-slate-500 uppercase mb-2">Target Type</span>
                  <div className="grid grid-cols-2 gap-2">
                    {(
                      [
                      ...
                      <button
                        ...
                        className={`px-3 py-2 rounded-lg border text-xs font-mono transition-all ${
                          targetType === t.key ? "bg-sky-100 text-sky-700 border-sky-300" : "bg-white border-slate-200 text-slate-700 hover:border-sky-300"
                        }`}
                      >
                        {t.label}
                      </button>
            ...
            <div>
              <span className="block text-xs font-mono text-slate-500 uppercase mb-2">
                ...
                {metricKind === "external" && <span className="text-slate-400 normal-case ml-1">— total, not per-pod</span>}
              </span>
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-[84px] text-right">
                  ...
                  {metricKind === "resource" && targetType === "averageValue" && (
                    <span className="text-[10px] font-mono text-slate-500 ml-0.5">{resourceName === "cpu" ? "m" : "Mi"}</span>
                  )}
                  {metricKind !== "resource" && <span className="text-[10px] font-mono text-slate-500 ml-0.5">/s</span>}
                </div>
              </div>
            ...
            {/* kubectl view */}
            <div className="p-3 rounded-xl bg-sky-50 border border-sky-200 font-mono text-[11px] text-sky-900 leading-relaxed overflow-x-auto">
              <span className="text-sky-700 font-bold">$ kubectl get hpa web-api -w</span>
              <br />
              NAME&nbsp;&nbsp;&nbsp;&nbsp;REFERENCE&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;TARGETS&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;MIN&nbsp;&nbsp;MAX&nbsp;&nbsp;REPLICAS
              <br />
              <span className="font-bold text-slate-900">web-api</span>&nbsp;&nbsp;Deployment/web-api&nbsp;&nbsp;
              <span className="text-sky-700 font-bold">{targetLabel}</span>&nbsp;&nbsp;{effectiveMin}&nbsp;&nbsp;&nbsp;{effectiveMax}&nbsp;&nbsp;&nbsp;{effectiveMin}
            </div>
          </div>
        </div>

        {/* Live YAML */}
        <div className="rounded-xl bg-white border border-sky-200 overflow-hidden flex flex-col min-h-[420px]">
          <div className="flex items-center justify-between px-5 py-3 border-b border-sky-100 bg-gradient-to-r from-blue-50 to-sky-50/40">
            <span className="text-xs font-mono text-slate-900 font-bold">hpa.yaml — autoscaling/v2</span>
            <button
              onClick={() => {
              ...
              className="px-3 py-1 rounded bg-white hover:bg-sky-50 border border-sky-200 text-xs font-mono text-slate-900 transition-colors"
            >
              {copiedHpa ? <span className="text-emerald-600">✓ Copied!</span> : <span>📋 Copy YAML</span>}
            </button>
          </div>
          <div className="p-4 font-mono text-xs text-slate-900 bg-slate-50 overflow-x-auto flex-1 whitespace-pre leading-relaxed">{hpaYaml}</div>
        </div>
      </div>

      {/* ======================= MODULE 2: SCALE BEHAVIOR EDITOR ======================= */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="p-5 rounded-xl bg-white border border-sky-200 space-y-4">
          <div className="flex items-center justify-between border-b border-sky-100 pb-3">
            <div className="flex items-center gap-2">
              <span className="text-base">🎚️</span>
              <div>
                <h3 className="text-sm font-bold text-slate-900">Scale-Up / Scale-Down Behavior Editor</h3>
                <p className="text-[11px] text-slate-500">Stabilization windows + per-event rate policies</p>
              </div>
            </div>
            <button
              onClick={() => {
              ...
              className="px-3 py-1 rounded bg-slate-50 hover:bg-sky-50 border border-slate-200 text-xs font-mono text-slate-900 transition-colors"
            >
              {copiedBehavior ? <span className="text-emerald-600">✓</span> : <span>📋</span>}
            </button>
          </div>
          ...
          <div className="p-3.5 rounded-xl border border-sky-200 bg-sky-50/40">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-mono font-bold text-sky-700">▲ Scale-Up</span>
              <span className="text-[10px] font-mono text-slate-500">window {behavior.upWindow}s · select {behavior.upSelect}</span>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <label className="block">
                <span className="text-[10px] font-mono text-slate-500 uppercase">Stabilization (s)</span>
                <input type="number" min={0} max={900} step={15} value={behavior.upWindow}
                  ...
                  className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-slate-200 text-xs font-mono focus:outline-none focus:border-sky-400" />
              </label>
              <label className="block">
                <span className="text-[10px] font-mono text-slate-500 uppercase">Max % per event</span>
                <input type="number" min={10} max={500} step={10} value={behavior.upPct}
                  onChange={(e) => setBh({ upPct: Math.max(10, Number(e.target.value) || 10) })}
                  className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-slate-200 text-xs font-mono focus:outline-none focus:border-sky-400" />
              </label>
              <label className="block">
                <span className="text-[10px] font-mono text-slate-500 uppercase">Max pods / event</span>
                <input type="number" min={1} max={20} value={behavior.upPods}
                  onChange={(e) => setBh({ upPods: Math.max(1, Number(e.target.value) || 1) })}
                  className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-slate-200 text-xs font-mono focus:outline-none focus:border-sky-400" />
              </label>
              <label className="block">
                <span className="text-[10px] font-mono text-slate-500 uppercase">Period (s)</span>
            ...
            {(["Max", "Min"] as const).map((sel) => (
              <button
                ...
                className={`mr-2 px-3 py-1.5 rounded-lg text-xs font-mono border transition-all ${
                  behavior.upSelect === sel ? "bg-sky-500 text-white border-sky-500" : "bg-white border-slate-200 text-slate-600 hover:border-sky-300"
                }`}
              >
                {sel}
              </button>
            ))}
            <span className="text-[10px] text-slate-500 ml-1">Max = most aggressive policy wins (fastest growth)</span>
          </div>

          {/* Scale-down panel */}
          <div className="p-3.5 rounded-xl border border-amber-200 bg-amber-50/30">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-mono font-bold text-amber-700">▼ Scale-Down</span>
              <span className="text-[10px] font-mono text-slate-500">window {behavior.downWindow}s · {behavior.downSelect === "Disabled" ? "DISABLED" : `selectPolicy ${behavior.downSelect}`}</span>
            </div>
            <div className="grid grid-cols-2 gap-3 mb-3">
              <label className="block">
                <span className="text-[10px] font-mono text-slate-500 uppercase">Stabilization (s)</span>
                <input type="number" min={0} max={1800} step={30} value={behavior.downWindow}
                  ...
                  className="mt-1 w-full px-2.5 py-1.5 rounded-lg border border-slate-200 text-xs font-mono focus:outline-none focus:border-amber-400" />
              </label>
              <label className="block">
                <span className="text-[10px] font-mono text-slate-500 uppercase">Max % per event</span>
            ...
            <span className="text-[10px] text-slate-500 ml-1">Disabled = never shrink pods</span>
          </div>

          <div className="p-3 rounded-xl bg-sky-50 border border-sky-200 text-[11px] text-slate-600 leading-relaxed">
            <span className="font-bold text-slate-900">Pro tip:</span> keep scale-down slower than scale-up — a 300s down window absorbs
            traffic dips that would otherwise trigger ping-pong scaling.
          </div>
        </div>

        {/* Behavior YAML */}
        <div className="rounded-xl bg-white border border-sky-200 overflow-hidden flex flex-col min-h-[320px]">
          <div className="flex items-center justify-between px-5 py-3 border-b border-sky-100 bg-gradient-to-r from-amber-50 to-sky-50/40">
            <span className="text-xs font-mono text-slate-900 font-bold">behavior block</span>
            <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-amber-100/60 text-amber-700">scaleDown.selectPolicy: {behavior.downSelect}</span>
          </div>
          <div className="p-4 font-mono text-xs text-slate-900 bg-slate-50 overflow-x-auto flex-1 whitespace-pre leading-relaxed">{behaviorYaml}</div>
        </div>
      </div>

      {/* ======================= MODULE 3: TARGET UTILIZATION CALCULATOR ======================= */}
      <div className="p-5 rounded-xl bg-white border border-sky-200">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-sky-100 pb-3 mb-4">
          <div className="flex items-center gap-2">
            <span className="text-base">🧮</span>
            <div>
              <h3 className="text-sm font-bold text-slate-900">Target Utilization → Required Replicas Calculator</h3>
              <p className="text-[11px] text-slate-500">ceil( replicas × observed ÷ target ) — the math HPA runs every reconcile loop</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-mono border transition-all ${
                calcMode === "resource" ? "bg-sky-100 border-sky-300 text-sky-700 font-bold" : "bg-white border-slate-200 text-slate-500"
              }`}
            >
              CPU / MEM
            </button>
            <button
              ...
              className={`px-3 py-1 rounded-lg text-xs font-mono border transition-all ${
                calcMode === "custom" ? "bg-sky-100 border-sky-300 text-sky-700 font-bold" : "bg-white border-slate-200 text-slate-500"
              }`}
            >
              Custom QPS
            </button>
          ...
          {calcMode === "resource" ? (
            <>
              ...
              <div className="md:col-span-1 xl:col-span-2">
                <span className="block text-xs font-mono text-slate-500 uppercase mb-2">Target Utilization</span>
                <div className="flex items-center gap-2 pt-1.5">
                  <input
                    ...
                    className="flex-1 accent-sky-500"
                  />
                  <span className="text-sm font-mono text-sky-800 min-w-[40px] text-right">{calcTarget}%</span>
                </div>
              </div>
            </>
          ) : (
        ...
        <div className="mt-4 p-3 rounded-xl bg-sky-50 border border-sky-200 text-[11px] text-slate-600 font-mono leading-relaxed">
          <span className="text-sky-700 font-bold">example:</span> {calcReplicas} pods currently at{" "}
      ...
      <div className="rounded-xl bg-white border border-blue-200 overflow-hidden">
        <div className="px-5 py-3 border-b border-blue-100 bg-gradient-to-r from-blue-50 to-sky-50/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <span className="text-base">📊</span>
            <div>
              <h3 className="text-sm font-bold text-slate-900">Vertical Pod Autoscaler — Recommendation Viewer</h3>
              <p className="text-[11px] text-slate-500">VPA mines history (histograms) and emits request bounds, not just a target</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="px-2.5 py-1.5 rounded-lg border border-sky-200 bg-white text-xs font-mono text-sky-800 focus:outline-none focus:border-sky-400"
            >
        ...
        <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="space-y-3">
            <span className="block text-xs font-mono text-slate-500 uppercase">Workload Profiles</span>
            {VPA_WORKLOADS.map((w) => (
              <button
                key={w.id}
                onClick={() => setVpaWorkloadId(w.id)}
                className={`w-full p-3.5 rounded-xl border text-left transition-all ${
                  vpaWorkload.id === w.id ? "bg-blue-50 border-blue-300 ring-1 ring-blue-400" : "bg-white border-slate-200 hover:border-blue-300"
                }`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="text-sm font-bold text-slate-900">{w.name}</span>
                  <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-blue-100 text-blue-700">{w.id}</span>
                </div>
                <p className="text-[11px] text-slate-500 mt-1">{w.note}</p>
              </button>
            ))}
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="overflow-x-auto rounded-xl border border-slate-200">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-sky-50 text-sky-900 font-mono text-left">
                    ...
                    <th className="px-3 py-2.5 font-bold bg-sky-100">Recommended</th>
                ...
                <tbody className="font-mono text-slate-700">
                  <tr className="border-t border-slate-200">
                    <td className="px-3 py-2.5 font-bold text-slate-900">cpu</td>
                    ...
                    <td className="px-3 py-2.5 bg-sky-100 font-bold text-sky-900">{fmtCpu(vpaRec.cpu.target)}</td>
                    <td className="px-3 py-2.5">{fmtCpu(vpaRec.cpu.uncapped)}</td>
                    <td className="px-3 py-2.5">{fmtCpu(vpaRec.cpu.upper)}</td>
                    <td className={`px-3 py-2.5 ${vpaRec.cpuDeltaPct > 0 ? "text-emerald-600" : "text-rose-600"}`}>
                      {vpaRec.cpuDeltaPct > 0 ? `−${vpaRec.cpuDeltaPct}%` : `+${-vpaRec.cpuDeltaPct}%`}
                    </td>
                  </tr>
                  <tr className="border-t border-slate-200">
                    <td className="px-3 py-2.5 font-bold text-slate-900">memory</td>
                    ...
                    <td className="px-3 py-2.5 bg-sky-100 font-bold text-sky-900">{fmtMemMi(vpaRec.mem.target)}</td>
                    <td className="px-3 py-2.5">{fmtMemMi(vpaRec.mem.uncapped)}</td>
                    <td className="px-3 py-2.5">{fmtMemMi(vpaRec.mem.upper)}</td>
                    <td className={`px-3 py-2.5 ${vpaRec.memDeltaPct > 0 ? "text-emerald-600" : "text-rose-600"}`}>
              ...
              {(
                ...
                return (
                  <div key={row.label}>
                    ...
                    <div className="relative h-6 rounded-lg bg-slate-100 border border-slate-200">
                      <div
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg bg-white border border-slate-300 hover:border-sky-400 text-slate-700 text-xs font-mono transition-colors"
              >
                Revert
              </button>
            ...
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 font-mono text-[11px] text-slate-600 leading-relaxed overflow-x-auto">
              <span className="text-blue-700 font-bold">$ kubectl describe vpa {vpaWorkload.id}-vpa</span>
              <br />
              Status: <span className="text-emerald-600">Running</span> · target: {vpaWorkload.name.split(" (")[0]}
              <br />
            ...
            <button
              ...
              className="text-[11px] font-mono text-blue-700 hover:text-blue-800 underline underline-offset-2"
            >
              {showVpaYaml ? "▾ hide" : "▸ show"} vpa.yaml manifest
            </button>
            {showVpaYaml && (
              <div className="rounded-xl border border-blue-200 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-2.5 border-b border-blue-100 bg-blue-50/50">
                  <span className="text-xs font-mono text-slate-900 font-bold">vpa.yaml — autoscaling.k8s.io/v1</span>
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(vpaYaml);
                    }}
                    className="px-2.5 py-1 rounded bg-white hover:bg-blue-50 border border-blue-200 text-[10px] font-mono text-slate-900 transition-colors"
                  >
                    📋 Copy
                  </button>
                </div>
                <div className="p-4 font-mono text-xs text-slate-900 bg-slate-50 overflow-x-auto whitespace-pre leading-relaxed">{vpaYaml}</div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ======================= MODULE 5: AUTOSCALING SIMULATOR ======================= */}
      <div className="p-5 rounded-xl bg-white border border-sky-200">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-sky-100 pb-4 mb-4">
          <div className="flex items-center gap-2">
            <span className="text-base">⚙️</span>
            <div>
              <h3 className="text-sm font-bold text-slate-900">Autoscaling Simulator — Live Replica Decisions</h3>
              <p className="text-[11px] text-slate-500">Drives the configured metric through HPA math: formula → stabilization → policy → min/max bounds</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-mono font-bold transition-colors ${
                running ? "bg-slate-200 hover:bg-slate-300 text-slate-700" : "bg-emerald-600 hover:bg-emerald-700 text-white"
              }`}
            >
            ...
            <select
              ...
              className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-mono text-slate-700 focus:outline-none"
            >
            ...
            {chart ? (
              ...
              <div className="py-16 text-center">
                ...
                <p className="text-xs font-mono text-slate-400">
                  Press <span className="text-sky-600 font-bold">▶ Simulate</span> to watch the HPA compute replicas against the {SCENARIOS.find((s) => s.key === scenarioKey)?.label.toLowerCase()} scenario
                </p>
              </div>
            )}
            <div className="flex items-center gap-4 px-1 pt-2 text-[10px] font-mono text-slate-500">
        ...
        <div className="mt-4 rounded-xl border border-sky-100 bg-sky-50/40 p-4">
          <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
            <span className="text-xs font-mono text-slate-700 font-bold">REPLICA COUNT VISUALIZATION — LAST {timeline.visible.length} TICKS</span>
          ...
          {timeline.visible.length === 0 ? (
            <p className="text-[11px] text-slate-400 font-mono">No samples yet — start the simulation.</p>
          ) : (
            <div className="flex flex-wrap gap-1">
              {timeline.visible.map((s, i) => (
                <div key={i} className="flex flex-col-reverse items-center gap-0.5">
                  <span className="text-[8px] font-mono text-slate-400 leading-none">{(s.t / 60).toFixed(0)}</span>
                  <div className="flex flex-col-reverse gap-px" title={`t=${(s.t / 60).toFixed(1)}m · ${s.replicas} pods (desired ${s.desired})`}>
                    ...
                    {s.replicas > timeline.cap && (
                      <span className="text-[8px] font-mono text-sky-600 text-center leading-none">+{s.replicas - timeline.cap}</span>
                    )}
                  </div>
                </div>
              ))}
        ...
        {/* Event log */}
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-900 p-4 font-mono text-[11px] leading-relaxed">
          <div className="flex items-center justify-between mb-3">
            <span className="text-slate-300 font-bold">SCALING EVENT LOG</span>
            <span className="text-slate-500">{events.length} events</span>
          </div>
          {events.length === 0 ? (
            <p className="text-slate-500">— waiting for the first scale event —</p>
          ) : (
            <div className="space-y-1">
              {events.slice(-10).map((ev, i) => (
                <div key={i} className="flex gap-3">
                  <span className="text-slate-600 w-14 shrink-0">t+{(ev.t / 60).toFixed(0)}m</span>
                  <span className={ev.to > ev.from ? "text-sky-400" : "text-amber-400"}>
                    {ev.to > ev.from ? "▲ SCALE UP" : "▼ SCALE DOWN"} {ev.from} → {ev.to}
                  </span>
                  <span className="text-slate-400 truncate">{ev.reason}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {/* ======================= BEST PRACTICES ======================= */}
      <div className="p-5 rounded-xl bg-gradient-to-br from-sky-50 via-white to-blue-50/50 border border-sky-200">
        <h3 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2">✅ Autoscaling Best Practices</h3>
        <ul className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-slate-600">
          <li className="flex gap-2"><span className="text-sky-600">▸</span> Start with <b>CPU utilization</b> (~70–80%), add custom metrics (QPS, queue depth) only once they stabilize.</li>
          <li className="flex gap-2"><span className="text-sky-600">▸</span> Set <b>minReplicas ≥ 2</b> for HA — the floor keeps serving even at zero load.</li>
          <li className="flex gap-2"><span className="text-sky-600">▸</span> Scale down gently: <b>stabilizationWindowSeconds ≥ 300s</b> prevents flapping on dips.</li>
          <li className="flex gap-2"><span className="text-sky-600">▸</span> HPA scales on <b>per-pod averages</b>; per-pod skew is handled by VPA instead.</li>
          <li className="flex gap-2"><span className="text-sky-600">▸</span> VPA on <b>memory</b> + HPA on <b>CPU</b> is battle-tested; avoid both on the same metric in Auto mode.</li>
          <li className="flex gap-2"><span className="text-sky-600">▸</span> After VPA changes requests, HPA's utilization denominator changes — re-check targets.</li>
        </ul>
      </div>
    </section>
  );
