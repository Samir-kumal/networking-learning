...
export default function AwsDrSection() {
  ...
  return (
    <section id="aws-dr" className="scroll-mt-20 space-y-6">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-[#161b22] via-[#1c2333] to-[#161b22] border border-slate-200 dark:border-slate-700 p-6 sm:p-10 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            ...
            <p className="mt-3 text-sm text-slate-400 dark:text-slate-500 max-w-2xl leading-relaxed">
      ...
      <div
        id="dr-objectives"
        className="scroll-mt-24 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 sm:p-8 space-y-8 shadow-xl hover:border-[#f0803e]/40 transition-colors"
      >
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
          <strong className="text-amber-600 dark:text-amber-400">RTO (Recovery Time Objective)</strong> is the maximum
          downtime you can tolerate — how fast service must be back.{" "}
          <strong className="text-amber-600 dark:text-amber-400">RPO (Recovery Point Objective)</strong> is the maximum
          acceptable data loss, i.e. how far back in time your last good copy may be. Together they
          set the severity of the disaster:{" "}
          <code className="text-xs bg-slate-50 dark:bg-slate-700 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300">
            Total Downtime = RPO + RTO
          </code>
        </p>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 bg-slate-50 dark:bg-slate-700 p-5 rounded-xl border border-slate-200 dark:border-slate-700">
          {/* RTO slider */}
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label
                htmlFor="rto-slider"
                className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase font-mono"
              >
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="px-3 py-1 rounded-lg bg-amber-500/15 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-600 text-sm font-mono font-bold">
            ...
            <div className="flex flex-wrap gap-1.5">
              {RPO_PRESETS.map((p) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-md text-[11px] font-mono border transition-colors ${
                    rpoMinutes === p.value
                      ? "bg-amber-500/15 text-amber-700 dark:text-amber-300 border-amber-400 dark:border-amber-500 font-bold"
                      : "bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-[#8b949e]"
                  }`}
                >
                  {p.label}
        ...
        <div className="rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-4 space-y-3">
          <div className="flex items-center justify-between text-xs font-mono border-b border-slate-200 dark:border-slate-700 pb-2">
            <span className="text-slate-900 dark:text-slate-100 font-bold">Strategy Feasibility Check</span>
            <span className="text-slate-500 dark:text-slate-400">Best case must be ≤ your target</span>
          </div>
          ...
          {recommended ? (
            <p className="text-xs text-slate-500 dark:text-slate-400">
              <strong className="text-emerald-600 dark:text-emerald-400">Best fit for your targets:</strong>{" "}
      ...
      >
          ...
          {STRATEGY_ORDER.map((k) => {
            ...
            return (
              <button
                ...
                className={`text-left rounded-xl border p-4 transition-all ${
                  active
                    ...
                    : "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:border-[#f0803e]/50"
              ...
              >
                  ...
                  <span
                    className={`w-10 h-10 rounded-lg flex items-center justify-center text-xs font-mono font-bold ${
                      active
                        ...
                        : "bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700"
                    }`}
                  >
                    {s.code}
                  </span>
                ...
                <h3 className="text-sm font-extrabold text-slate-900 dark:text-slate-100">{s.label}</h3>
                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 leading-snug">{s.tagline}</p>
                <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 space-y-1 text-[11px] font-mono">
                  <div className="flex justify-between">
                    <span className="text-slate-400 dark:text-slate-500">RTO</span>
                    <span className="text-slate-700 dark:text-slate-300 font-bold">
                      {formatMinutes(s.rtoRange[0])} – {formatMinutes(s.rtoRange[1])}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400 dark:text-slate-500">RPO</span>
                    <span className="text-slate-700 dark:text-slate-300 font-bold">
                      {formatMinutes(s.rpoRange[0])} – {formatMinutes(s.rpoRange[1])}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400 dark:text-slate-500">Cost tier</span>
                    <span className="text-amber-600 dark:text-amber-400 font-bold">{s.costTier}/mo</span>
                  </div>
                </div>
              </button>
            );
        ...
        <div className="rounded-xl bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 p-5 space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-700 pb-3">
            <h3 className="text-xs font-extrabold text-slate-900 dark:text-slate-100 font-mono">
              {strategy.label}
              <span className="ml-2 px-2 py-0.5 rounded bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-[10px] uppercase">
            ...
            <span className="text-xs font-mono text-slate-500 dark:text-slate-400">
              ...
              {meetsTarget(strategy) ? (
                <span className="text-emerald-600 dark:text-emerald-400 font-bold">this strategy fits</span>
              ) : (
                <span className="text-rose-600 dark:text-rose-400 font-bold">too slow for your targets — pick warmer</span>
              )}
            </span>
          </div>

          <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{strategy.description}</p>

          <div>
            <span className="text-[11px] font-mono uppercase text-slate-400 dark:text-slate-500 font-bold block mb-2">
              Key AWS Services
            </span>
            <div className="flex flex-wrap gap-2">
              {strategy.awsServices.map((svc) => (
                <span
                  key={svc}
                  className="px-2.5 py-1 rounded-md bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-700 text-[11px] font-mono"
                >
                  {svc}
                </span>
              ))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3">
              <span className="text-[10px] font-mono uppercase text-slate-400 dark:text-slate-500">Failover</span>
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">{strategy.failover}</p>
            </div>
            <div className="rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3">
              <span className="text-[10px] font-mono uppercase text-slate-400 dark:text-slate-500">Data recovery</span>
              <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 leading-relaxed">{strategy.dataRecovery}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="rounded-xl bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 p-4">
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-xs font-mono">
            <span className="text-slate-500 dark:text-slate-400">
              RPO <span className="text-rose-600 dark:text-rose-400 font-bold">{formatMinutes(rpoMinutes)}</span>
            </span>
        ...
        <div className="space-y-2">
          <div className="w-full h-10 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 flex">
            <div
              style={{ width: `${lossPct}%` }}
              className="bg-rose-200 dark:bg-rose-900/40 border-r-2 border-rose-400 dark:border-rose-500 flex items-center justify-center overflow-hidden"
              title={`Data loss window (RPO): ${formatMinutes(rpoMinutes)}`}
            >
              <span className="text-[10px] font-mono font-bold text-rose-700 dark:text-rose-300 whitespace-nowrap px-1">
                DATA LOSS ({formatMinutes(rpoMinutes)})
              </span>
            </div>
            <div
              style={{ width: `${recoveryPct}%` }}
              className="bg-amber-200 dark:bg-amber-900/40 flex items-center justify-center overflow-hidden"
              title={`Recovery window (RTO): ${formatMinutes(rtoMinutes)}`}
            >
              <span className="text-[10px] font-mono font-bold text-amber-700 dark:text-amber-300 whitespace-nowrap px-1">
          ...
          <div className="relative h-8">
            <div
              className="absolute top-0 bottom-0 border-l-2 border-dashed border-rose-400 dark:border-rose-500"
              style={{ left: `${lossPct}%` }}
            />
            <div className="absolute top-0 -translate-x-full">
              <span className="text-[10px] font-mono text-slate-600 dark:text-slate-300 whitespace-nowrap">
                T+0 disaster
              </span>
            </div>
            <div
              ...
              style={{ left: `${lossPct}%` }}
            >
              <span className="text-[10px] font-mono text-rose-600 dark:text-rose-400 bg-white dark:bg-slate-800 px-1.5 py-0.5 rounded border border-rose-200 dark:border-rose-700 whitespace-nowrap">
                Recovery point (T – {formatMinutes(rpoMinutes)})
              </span>
            </div>
            <div className="absolute top-0 right-0">
              <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400 bg-white dark:bg-slate-800 px-1.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-700 whitespace-nowrap">
          ...
          {[
            {
              ...
              cls: "border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400",
            },
            {
              step: "02",
              title: "Failover",
              desc: strategy.failover,
              cls: "border-amber-200 dark:border-amber-700 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400",
            },
            {
              step: "03",
              title: "Recover data",
            ...
            },
            {
              ...
              cls: "border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400",
            },
          ].map((p) => (
            <div key={p.step} className={`rounded-xl border p-4 ${p.cls}`}>
              ...
              <h4 className="text-sm font-extrabold text-slate-900 dark:text-slate-100 mt-0.5">{p.title}</h4>
              <p className="text-[11px] text-slate-600 dark:text-slate-300 mt-1.5 leading-relaxed">{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-xl p-4">
            ...
            <div className="flex flex-wrap gap-4 mt-2 text-[11px] font-mono text-slate-500 dark:text-slate-400">
            ...
            <div className="rounded-xl bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-700 p-5 space-y-4">
              ...
              {(
                [
                  ...
                  { key: "storage", label: "Storage & backups", color: "bg-amber-200 dark:bg-amber-900/40" },
                ] as const
              ).map((row) => {
                ...
                return (
                  <div key={row.key}>
                    ...
                    <div className="h-2.5 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <div className="flex justify-between pt-3 border-t border-slate-200 dark:border-slate-700 text-[11px] font-mono text-slate-500 dark:text-slate-400">
                <span>Annual TCO projection</span>
                <span className="text-slate-900 dark:text-slate-100 font-bold">${annualCost.toLocaleString()}/yr</span>
              </div>
            </div>
        ...
        <div className="rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 overflow-hidden">
          <div className="border-b border-slate-200 dark:border-slate-700 px-4 py-3">
            <h4 className="text-xs font-mono font-bold text-slate-900 dark:text-slate-100">Strategy Comparison Matrix</h4>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400 border-b border-slate-200 dark:border-slate-700">
                  ...
                  return (
                    <tr
                      ...
                      className={`border-b border-slate-100 dark:border-slate-700 last:border-0 transition-colors ${
                        selected ? "bg-[#f0803e]/5" : "hover:bg-slate-50/50 dark:hover:bg-slate-700/50"
                      }`}
                    >
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <span className="text-amber-600 dark:text-amber-400 font-bold">{s.costTier}</span>
                        <span className="text-slate-400 dark:text-slate-500">/mo · {(s.baseCost / 1000).toFixed(1)}k</span>
                      </td>
                      <td className="p-3 text-slate-500 dark:text-slate-400">{s.tagline}</td>
                    </tr>
                  );
                })}
              </tbody>
