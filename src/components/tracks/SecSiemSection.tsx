  ...
  return (
    <section id="sec-siem" className="scroll-mt-20 space-y-6">
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <h3 className="mt-2 text-lg font-bold text-slate-900">Event filtering and detection rules</h3>
      ...
      <form onSubmit={handleDetect} className="space-y-6">
        <div className="grid grid-cols-1 gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm md:grid-cols-[minmax(0,1.4fr)_minmax(0,0.8fr)_minmax(0,0.8fr)_auto] md:items-end">
          ...
          <button type="submit" className="rounded-lg bg-cyan-600 px-4 py-2.5 text-xs font-semibold text-white transition-colors hover:bg-cyan-700">Run detection</button>
        </div>

        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
            <div><h4 className="text-sm font-bold text-slate-900">Structured event stream</h4><p className="mt-1 text-xs text-slate-500">Synthetic events only; source addresses use documentation ranges.</p></div>
            <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-mono font-semibold text-slate-600">{filteredEvents.length} of {EVENTS.length} events</span>
          </div>
          <div className="overflow-x-auto rounded-lg border border-slate-200">
            <table className="w-full min-w-[980px] text-left text-[11px]">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredEvents.map((event) => <tr key={event.id} className="text-slate-600"><td className="px-3 py-2 font-mono font-semibold text-slate-800">{event.id}</td><td className="px-3 py-2">{event.source}</td><td className="px-3 py-2"><span className="rounded-full bg-slate-100 px-2 py-0.5 font-semibold uppercase text-slate-600">{event.severity}</span></td><td className="max-w-[260px] px-3 py-2"><span className="block font-mono text-slate-700">{event.kind}</span><span className="mt-0.5 block text-slate-500">{event.summary}</span></td><td className="px-3 py-2 font-mono">{event.user}</td><td className="px-3 py-2 text-center">{event.failedAttempts || "—"}</td><td className="px-3 py-2 text-center">{event.privilegeChange ? "Yes" : "—"}</td><td className="px-3 py-2 font-mono">{formatBytes(event.bytesOut)}</td></tr>)}
                {filteredEvents.length === 0 && <tr><td colSpan={8} className="px-3 py-8 text-center text-xs text-slate-500">No events match the selected filters.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        {result ? (
          <div role="status" aria-live="polite" className={`rounded-xl border p-5 shadow-sm ${result.matchingIds.length > 0 ? "border-cyan-200 bg-cyan-50" : "border-slate-200 bg-slate-50"}`}>
            <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-wide text-cyan-700">Evaluator-backed detection result</p><h4 className="mt-1 text-base font-bold text-slate-900">{result.matchingIds.length > 0 ? `${result.matchingIds.length} matching event${result.matchingIds.length === 1 ? "" : "s"}` : "No matching events"}</h4></div><span className="rounded-full bg-white px-3 py-1.5 text-xs font-mono font-bold text-cyan-800">{selectedRule.label}</span></div>
            <dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2"><div><dt className="font-semibold text-slate-700">Matching event IDs</dt><dd className="mt-0.5 font-mono text-slate-600">{result.matchingIds.length > 0 ? result.matchingIds.join(", ") : "None"}</dd></div><div><dt className="font-semibold text-slate-700">Analyst conclusion</dt><dd className="mt-0.5 leading-relaxed text-slate-600">{result.matchingIds.length > 0 ? `Investigate ${result.matchingIds.join(", ")} first, enrich the alert with identity and asset context, and preserve the correlated records.` : "The selected rule found no matches in this filtered scope; widen the source or severity filter before closing the query."}</dd></div></dl>
          </div>
        ) : (
          <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-xs text-slate-500">Choose a rule and optional filters, then run detection to see evaluator-matched event IDs and the analyst conclusion.</p>
        )}
      </form>
    </section>
  );
